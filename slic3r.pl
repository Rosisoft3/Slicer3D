...
if (@ARGV) {  # slicing from command line
    ...
    while (my $input_file = shift @ARGV) {
        ...
        if ($opt{export_svg}) {
			...
			printf "Filament Required Length(mm): %.2f\n", $sprint->total_used_filament;
			printf "Filament Required Volume(cm^3): %.2f\n", $sprint->total_extruded_volume/1000;
			printf "Filament Required Weight(g): %.2f \n", $sprint->total_weight;
			printf "Total Filament Cost: %.2f\n", $sprint->total_cost;
			printf "Estimated Printing Time: ". $sprint->estimated_print_time." \n";
